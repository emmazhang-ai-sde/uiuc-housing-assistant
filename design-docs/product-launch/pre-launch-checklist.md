...
- **"Before User Created" Auth Hook — confirmed NOT configured.** Checked the live Supabase Dashboard (Authentication → Hooks) directly: the Hooks list is empty. `design-docs/user-authentication.md` §5.6 already flagged this as unverified ("if the dashboard has no Before User Created hook, stop and reassess"); this confirms the pessimistic case. Only the client-side `is_email_on_waitlist` RPC gate is live today — someone bypassing the frontend and calling `signInWithOtp` directly with an off-waitlist email could still create an account. Not fixed yet (user chose to defer backend work during this session).
- **New risk from today's Sign Up/Log In merge** — the waitlist RPC check now runs on every login attempt, not just first-time signup (previously "Log In" mode skipped it entirely once an account existed). This is correct per the original §5.4 design, but it means **any already-authenticated account whose email is not currently on the `waitlist` table will be locked out the next time its session expires and it has to re-authenticate** — including any personal/test accounts (e.g. the mail-merge test addresses) that were never actually inserted into `waitlist`, only used to test email delivery. Worth confirming before wider rollout that every account you expect to keep working (your own included) is actually present in the `waitlist` table, not just previously-invited via Supabase Auth.
...
- [ ] Add the "Before User Created" Auth Hook — confirmed empty in the live Supabase Dashboard; only the soft client-side RPC gate is active today
- [ ] Confirm every account that needs to keep working (incl. your own) is actually present in the `waitlist` table — the Sign Up/Log In merge made the waitlist check apply to every login, not just first signup
